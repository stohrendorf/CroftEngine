...
namespace gameplay
{
    ...
    std::shared_ptr<Image> Image::createRGBA(unsigned int width, unsigned int height, const char* data)
    {
        BOOST_ASSERT(width > 0 && height > 0);
        ...
        for(size_t i = 0; i < width*height; ++i, d += 4)
            image->_data[i] = glm::vec4{ d[0] / 255.0f, d[1] / 255.0f, d[2] / 255.0f, d[3] / 255.0f };
