...
namespace gameplay
{
    ...
    void Curve::interpolateQuaternion(float s, float* from, float* to, float* dst) const
    {
        ...
        glm::quat tmp;
        if( s >= 0 )
            tmp = glm::slerp(glm::quat{ from[0], from[1], from[2], from[3] }, glm::quat{ to[0], to[1], to[2], to[3] }, s);
        else
            tmp = glm::slerp(glm::quat{ to[0], to[1], to[2], to[3] }, glm::quat{ from[0], from[1], from[2], from[3] }, s);
