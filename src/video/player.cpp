...
struct FilterGraph
{
    ...
    void init(const Stream& stream)
    {
        ...
        if(avfilter_graph_create_filter(&input, avfilter_get_by_name("buffer"), "in", filterGraphArgs, nullptr, graph)
           < 0)
        {
            throw std::runtime_error("Cannot create buffer source");
        }

        if(avfilter_graph_create_filter(&output, avfilter_get_by_name("buffersink"), "out", nullptr, nullptr, graph)
           < 0)
        {
