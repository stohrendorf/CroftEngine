...
struct FilterGraph
{
    ...
    void init(const Stream& stream)
    {
        ...
        snprintf( filterGraphArgs, sizeof( filterGraphArgs ),
        ...
        if( avfilter_graph_create_filter( &input, avfilter_get_by_name( "buffer" ), "in", filterGraphArgs, nullptr,
                                          graph ) < 0 )
        {
            throw std::runtime_error( "Cannot create buffer source" );
        }

        if( avfilter_graph_create_filter( &output, avfilter_get_by_name( "buffersink" ), "out", nullptr, nullptr,
                                          graph ) < 0 )
        {
    ...
    void decodePacket()
    {
        ...
        else if( packet.stream_index == audioStream->index )
        {
            ...
            while( (err = avcodec_receive_frame( audioStream->context, audioFrame.frame )) == 0 )
            {
                ...
                const auto framesDecoded = swr_convert( swrContext, &audioData, outSamples,
...
struct Scaler
{
    ...
    void resize(int targetWidth, int targetHeight)
    {
        ...
        av_freep( dstVideoData );
        if( av_image_alloc( dstVideoData, dstVideoLinesize,
    ...
    void scale(const AVFramePtr& videoFrame, render::gl::Image<render::gl::RGBA8>& img)
    {
        sws_scale( context, static_cast<const uint8_t* const*>(videoFrame.frame->data),
                   videoFrame.frame->linesize, 0, videoFrame.frame->height, dstVideoData, dstVideoLinesize );

        auto srcLineRaw = dstVideoData[0];
        auto dst = img.getRawData();

        Expects( img.getWidth() <= dstVideoLinesize[0] / sizeof( render::gl::RGBA8 ) );

        img.fill( {0, 0, 0, 255} );

        Expects( img.getWidth() >= scaledWidth );
        Expects( img.getHeight() >= scaledHeight );
        const auto xOffset = static_cast<GLint>((img.getWidth() - scaledWidth) / 2);
        const auto yOffset = static_cast<GLint>((img.getHeight() - scaledHeight) / 2);
        dst += img.getWidth() * yOffset;
        dst += xOffset;
        for( GLint y = 0; y < scaledHeight; ++y )
        {
            std::copy_n( reinterpret_cast<render::gl::RGBA8*>(srcLineRaw), scaledWidth, dst );
            dst += img.getWidth();
