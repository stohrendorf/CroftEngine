...
template<typename T>
struct HalfOpenInterval final
{
  T min{};
  T max{};

  explicit HalfOpenInterval() = default;
  HalfOpenInterval(const HalfOpenInterval<T>&) = default;
  HalfOpenInterval(HalfOpenInterval<T>&&) noexcept = default;

  HalfOpenInterval(const T& min, const T& max)
      : min{min}
      , max{max}
  {
  }

  constexpr auto& operator=(const HalfOpenInterval<T>& rhs)
  {
    min = rhs.min;
    max = rhs.max;
    return *this;
  ...
  [[nodiscard]] constexpr bool operator==(const HalfOpenInterval<T>& rhs)
  {
    return min == rhs.min && max == rhs.max;
  }
  ...
  [[nodiscard]] constexpr auto operator+(const T& value) const
  {
    return HalfOpenInterval<T>{min + value, max + value};
  }
  ...
  [[nodiscard]] constexpr auto intersect(const HalfOpenInterval<T>& rhs) const
  {
    return HalfOpenInterval<T>{std::max(min, rhs.min), std::min(max, rhs.max)};
  }
};
} // namespace core
