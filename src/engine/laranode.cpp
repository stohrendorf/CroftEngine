...
namespace engine
{
                              ...
                              const core::TRRotationXY& aimAngle)
    {
        ...
        if( !cylinders.empty() )
        {
            ...
            {
                const float proj = dot(cylinder.position.toRenderSystem() - start, dir) / length2(dir);
                if( proj < 0 )
                    hitPos = start;
                else if( proj > 1 )
                    hitPos = end;
...
#define renderMesh(IDX) getSkeleton()->getChild(IDX)->setLocalMatrix(matrixStack.top())
        ...
        if( m_handStatus == HandStatus::Combat || m_handStatus == HandStatus::Unholster || m_handStatus == HandStatus::
            Holster )
        {
