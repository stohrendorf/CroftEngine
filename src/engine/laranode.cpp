...
const char* toString(const HandStatus s)
{
    switch(s)
    {
    case HandStatus::None: return "None";
    case HandStatus::Grabbing: return "Grabbing";
    case HandStatus::Unholster: return "Unholster";
    case HandStatus::Holster: return "Holster";
    case HandStatus::Combat: return "Combat";
    default: BOOST_THROW_EXCEPTION(std::domain_error("Invalid HandStatus"));
    }
}
...
const char* toString(const UnderwaterState s)
{
    switch(s)
    {
    case UnderwaterState::OnLand: return "OnLand";
    case UnderwaterState::Diving: return "Diving";
    case UnderwaterState::Swimming: return "Swimming";
    default: BOOST_THROW_EXCEPTION(std::domain_error("Invalid UnderwaterState"));
    }
}
...
const char* toString(const core::Axis a)
{
    switch(a)
    {
    case core::Axis::PosZ: return "+Z";
    case core::Axis::PosX: return "+X";
    case core::Axis::NegZ: return "-Z";
    case core::Axis::NegX: return "-X";
...
const char* toString(const LaraNode::WeaponId id)
{
    switch(id)
    {
    case LaraNode::WeaponId::None: return "None";
    case LaraNode::WeaponId::Pistols: return "Pistols";
    case LaraNode::WeaponId::AutoPistols: return "AutoPistols";
    case LaraNode::WeaponId::Uzi: return "Uzi";
    case LaraNode::WeaponId::Shotgun: return "Shotgun";
    default: BOOST_THROW_EXCEPTION(std::domain_error("Invalid WeaponId"));
    }
}
...
{
    getSkeleton()->setAnimation(m_state, &getEngine().getAnimation(anim), firstFrame.get_value_or(0_frame));
}
...
void LaraNode::drawRoutine()
{
    ...
    if(m_handStatus == HandStatus::Combat || m_handStatus == HandStatus::Unholster
       || m_handStatus == HandStatus::Holster)
    {
...
YAML::Node LaraNode::save() const
{
    ...
    node["yRotationSpeed"] = m_yRotationSpeed;
    node["movementAngle"] = m_movementAngle;
...
void LaraNode::load(const YAML::Node& n)
{
    ...
    m_yRotationSpeed = n["yRotationSpeed"].as<core::Angle>();
    m_movementAngle = n["movementAngle"].as<core::Angle>();
...
YAML::Node LaraNode::AimInfo::save(const engine::Engine& engine) const
{
    ...
    if(weaponAnimData != nullptr)
        node["animData"]
            = std::distance(&engine.getPoseFrames().front(), reinterpret_cast<const int16_t*>(weaponAnimData));
    node["frame"] = frame;
    node["aiming"] = aiming;
...
void LaraNode::AimInfo::load(const YAML::Node& n, const engine::Engine& engine)
{
    if(!n["animData"].IsDefined())
        ...
        weaponAnimData
            = reinterpret_cast<const loader::file::AnimFrame*>(&engine.getPoseFrames().at(n["animData"].as<size_t>()));
