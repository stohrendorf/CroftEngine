...
namespace engine
{
    ...
    public:
        ...
        void update()
        {
            ...
            if(m_controllerLayout.isButtonPressed(ControllerButton::DPAD_LEFT, gsl::span<const uint8_t>{controllerButtons, controllerButtonCount}))
                left = true;

            auto right = glfwGetKey(m_window, GLFW_KEY_D) == GLFW_PRESS;
            if(m_controllerLayout.isButtonPressed(ControllerButton::DPAD_RIGHT, gsl::span<const uint8_t>{controllerButtons, controllerButtonCount}))
                right = true;

            auto stepLeft = glfwGetKey(m_window, GLFW_KEY_Q) == GLFW_PRESS;
            if(m_controllerLayout.isButtonPressed(ControllerButton::LB, gsl::span<const uint8_t>{controllerButtons, controllerButtonCount}))
                stepLeft = true;

            auto stepRight = glfwGetKey(m_window, GLFW_KEY_E) == GLFW_PRESS;
            if(m_controllerLayout.isButtonPressed(ControllerButton::RB, gsl::span<const uint8_t>{controllerButtons, controllerButtonCount}))
                stepRight = true;

            auto forward = glfwGetKey(m_window, GLFW_KEY_W) == GLFW_PRESS;
            if(m_controllerLayout.isButtonPressed(ControllerButton::DPAD_UP, gsl::span<const uint8_t>{controllerButtons, controllerButtonCount}))
                forward = true;

            auto backward = glfwGetKey(m_window, GLFW_KEY_S) == GLFW_PRESS;
            if(m_controllerLayout.isButtonPressed(ControllerButton::DPAD_DOWN, gsl::span<const uint8_t>{controllerButtons, controllerButtonCount}))
                backward = true;

            m_inputState.moveSlow = glfwGetKey(m_window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS
                                    || glfwGetKey(m_window, GLFW_KEY_RIGHT_SHIFT) == GLFW_PRESS;
            if(m_controllerLayout.isButtonPressed(ControllerButton::B, gsl::span<const uint8_t>{controllerButtons, controllerButtonCount}))
            ...
            if(m_controllerLayout.isButtonPressed(ControllerButton::BACK, gsl::span<const uint8_t>{controllerButtons, controllerButtonCount}))
            ...
            {
                x += m_controllerLayout.getAxisValue(ControllerAxis::LS_X, gsl::span<const float>{controllerAxisValues, controllerAxisCount}, 0) * 500;
                y += m_controllerLayout.getAxisValue(ControllerAxis::LS_Y, gsl::span<const float>{controllerAxisValues, controllerAxisCount}, 0) * 500;
            }
            else
            {
                if(m_controllerLayout.getAxisValue(ControllerAxis::LS_Y, gsl::span<const float>{controllerAxisValues, controllerAxisCount}, 0) > AxisThreshold)
                    forward = true;
                if(m_controllerLayout.getAxisValue(ControllerAxis::LS_Y, gsl::span<const float>{controllerAxisValues, controllerAxisCount}, 0) < -AxisThreshold)
                    backward = true;
                if(m_controllerLayout.getAxisValue(ControllerAxis::LS_X, gsl::span<const float>{controllerAxisValues, controllerAxisCount}, 0) < -AxisThreshold)
                    left = true;
                if(m_controllerLayout.getAxisValue(ControllerAxis::LS_X, gsl::span<const float>{controllerAxisValues, controllerAxisCount}, 0) > AxisThreshold)
                    right = true;
            }
