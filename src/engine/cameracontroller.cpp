...
CameraController::CameraController(const gsl::not_null<Engine*>& engine,
    ...
    , m_engine{engine}
    , m_position{engine->getLara().m_state.position.room}
    , m_lookAt{core::RoomBoundPosition{engine->getLara().m_state.position.room,
                                       engine->getLara().m_state.position.position}}
    , m_positionYOffset{engine->getLara().m_state.position.position.Y - core::SectorSize}
{
...
std::unordered_set<const loader::file::Portal*> CameraController::update()
{
  ...
  objects::Object* const focusedObject = isCompletelyFixed ? m_lookAtObject.get() : &m_engine->getLara();
...
core::Length CameraController::moveIntoGeometry(core::RoomBoundPosition& pos, const core::Length& margin) const
{
  ...
  Expects(sector != nullptr && sector->box != nullptr);
...
void CameraController::chaseObject(const objects::Object& object, bool fixed)
{
  ...
  m_horizontalDistanceSq = util::square(dist);

  core::RoomBoundPosition eye(m_position->room);
  eye.position = {0_len, util::sin(m_distance, m_rotationAroundLara.X), 0_len};

  const core::Angle y = m_rotationAroundLara.Y + object.m_state.rotation.Y;
  eye.position += m_lookAt->position - util::pitch(dist, y);
  clampBox(eye,
           [this](core::Length& a,
                  core::Length& b,
                  const core::Length& c,
                  ...
                  const core::Length& f,
                  const core::Length& g,
                  const core::Length& h) { clampToCorners(m_horizontalDistanceSq, a, b, c, d, e, f, g, h); });
...
void CameraController::handleFreeLook()
{
  ...
  m_lookAt->position.X = m_engine->getLara().m_state.position.position.X;
  m_lookAt->position.Z = m_engine->getLara().m_state.position.position.Z;
  m_rotationAroundLara.X = m_engine->getLara().m_torsoRotation.X + m_engine->getLara().m_headRotation.X
                           + m_engine->getLara().m_state.rotation.X;
  m_rotationAroundLara.Y = m_engine->getLara().m_torsoRotation.Y + m_engine->getLara().m_headRotation.Y
                           + m_engine->getLara().m_state.rotation.Y;
  m_distance = core::DefaultCameraLaraDistance;
  m_positionYOffset = -util::sin(core::SectorSize / 2, m_rotationAroundLara.Y);
  m_lookAt->position += util::pitch(m_positionYOffset, m_engine->getLara().m_state.rotation.Y);
...
void CameraController::handleEnemy()
{
  ...
  if(m_enemy != nullptr)
  {
    m_rotationAroundLara.X = m_eyeRotation.X + m_engine->getLara().m_state.rotation.X;
    m_rotationAroundLara.Y = m_eyeRotation.Y + m_engine->getLara().m_state.rotation.Y;
  }
  else
  {
    m_rotationAroundLara.X = m_engine->getLara().m_torsoRotation.X + m_engine->getLara().m_headRotation.X
  ...
  auto center = *m_lookAt;
  const auto d = util::cos(m_distance, m_rotationAroundLara.X);
  center.position -= util::pitch(d, m_rotationAroundLara.Y);
  center.position.Y += util::sin(m_distance, m_rotationAroundLara.X);
  center.room = m_position->room;
...
void CameraController::serialize(const serialization::Serializer& ser)
{
  ser(S_NV("position", m_position),
      ...
      S_NV("horizontalDistanceSq", m_horizontalDistanceSq),
