...
public:
    explicit ControllerLayout(const std::string& configFilename)
    {
        ...
        {
            m_buttons[EnumUtil<ControllerButton>::fromString( button["name"].as<std::string>() )] = button["index"]
                    .as<int>();
        }
    ...
    bool isButtonPressed(const ControllerButton btn, const gsl::span<const uint8_t>& buttons) const
    {
    ...
    float getAxisValue(const ControllerAxis axis, const gsl::span<const float>& values, const float def) const
    {
