    ...
    : ItemNode{engine, room, item, hasUpdateFunction}
{
    ...
    m_node->addMaterialParameterSetter(
        "u_diffuseTexture",
        [texture = sprite.texture](const render::scene::Node& /*node*/, render::gl::Program::ActiveUniform& uniform) {
            uniform.set(*texture);
        });
    m_node->addMaterialParameterSetter(
        "u_lightAmbient",
                                    ...
                                    Engine& engine, const core::RoomBoundPosition&, core::Speed, core::Angle))
{
    ...
    roomPos.position
        = core::TRVec{glm::vec3{translate(itemSpheres.at(boneIndex).m, localPosition.toRenderSystem())[3]}};
