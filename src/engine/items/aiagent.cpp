...
bool AIAgent::anyMovingEnabledItemInReach() const
{
    for( const std::shared_ptr<ItemNode>& item : getLevel().m_itemNodes | boost::adaptors::map_values )
    {
        ...
        if( item->m_state.triggerState == items::TriggerState::Active
...
bool AIAgent::animateCreature(const core::Angle angle, core::Angle tilt)
{
    ...
    auto sector = getLevel()
            .findRealFloorSector( m_state.position.position + core::TRCoordinates{0, bbox.minY, 0}, &room );
    ...
    if( lotInfo.fly != 0 )
    {
        ...
        currentFloor = HeightInfo::fromFloor( sector, core::TRCoordinates{m_state.position.position.X, bboxMinY,
        ...
        else
        {
            const auto ceiling = HeightInfo::fromCeiling( sector,
                                                          core::TRCoordinates{m_state.position.position.X, bboxMinY,
                                                                              m_state.position.position.Z},
                                                          getLevel().m_itemNodes, getLevel().m_floorData ).distance;
