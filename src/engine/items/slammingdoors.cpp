    ...
    else
    {
        ...
        if(m_state.touch_bits.any() && m_state.current_anim_state == 1_as)
        {
            ...
            const auto emitBlood = [&itemSpheres, this](const core::TRVec& bitePos, size_t boneId) {
                const auto position
                    = core::TRVec{glm::vec3{translate(itemSpheres.at(boneId).m, bitePos.toRenderSystem())[3]}};

                auto blood = createBloodSplat(getEngine(),
                                              core::RoomBoundPosition{m_state.position.room, position},
                                              m_state.speed,
                                              m_state.rotation.Y);
