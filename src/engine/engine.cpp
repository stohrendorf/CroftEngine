...
{
    m_lightningShader
        = render::scene::ShaderProgram::createFromFile("shaders/lightning.vert", "shaders/lightning.frag");
    ...
    for(loader::file::Item& item : m_level->m_items)
    {
        ...
        if(const auto& model = findAnimatedModelForType(item.type))
        {
            ...
            else if(item.type == TR1ItemId::LionMale || item.type == TR1ItemId::LionFemale
        ...
        if(const auto& spriteSequence = findSpriteSequenceForType(item.type))
        {
            ...
            else if(item.type == TR1ItemId::Item141 || item.type == TR1ItemId::Item142
                    || item.type == TR1ItemId::Key1Sprite || item.type == TR1ItemId::Key2Sprite
                    || item.type == TR1ItemId::Key3Sprite || item.type == TR1ItemId::Key4Sprite
                    || item.type == TR1ItemId::Puzzle1Sprite || item.type == TR1ItemId::Puzzle2Sprite
                    || item.type == TR1ItemId::Puzzle3Sprite || item.type == TR1ItemId::Puzzle4Sprite
                    || item.type == TR1ItemId::PistolsSprite || item.type == TR1ItemId::ShotgunSprite
                    || item.type == TR1ItemId::MagnumsSprite || item.type == TR1ItemId::UzisSprite
                    || item.type == TR1ItemId::PistolAmmoSprite || item.type == TR1ItemId::ShotgunAmmoSprite
                    || item.type == TR1ItemId::MagnumAmmoSprite || item.type == TR1ItemId::UziAmmoSprite
                    || item.type == TR1ItemId::ExplosiveSprite || item.type == TR1ItemId::SmallMedipackSprite
                    || item.type == TR1ItemId::LargeMedipackSprite || item.type == TR1ItemId::ScionPiece2
    ...
    , m_inventory{*this}
{
    ...
    if(!isVideo)
    {
        ...
        m_audioEngine = std::make_unique<AudioEngine>(
            *this, m_level->m_soundDetails, m_level->m_soundmap, m_level->m_sampleIndices);
        ...
        for(size_t i = 0; i < m_level->m_textures.size(); ++i)
        {
            ...
            m_level->m_textures[i].toTexture(
                glidos.get(),
                std::function<void(const std::string&)>([this](const std::string& s) { drawLoadingScreen(s); }));
        }
