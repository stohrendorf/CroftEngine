...
std::optional<IndexCommand> parseIndex(const std::string& line)
{
  ...
  {
    if(const auto frame = parseTime(m[2]); frame.has_value())
...
std::optional<size_t> parsePregap(const std::string& line)
{
  ...
  {
    if(const auto frame = parseTime(m[1]); frame.has_value())
      return frame;
  }
...
std::vector<Track> readCueSheet(const std::filesystem::path& filename)
{
  ...
  {
    if(const auto trackCmd = parseTrack(line); trackCmd.has_value())
    {
    ...
    else if(const auto indexCmd = parseIndex(line); indexCmd.has_value())
    {
    ...
    else if(const auto fileCmd = parseFile(line); fileCmd.has_value())
    {
    ...
    else if(const auto pregapCmd = parsePregap(line); pregapCmd.has_value())
    {
    ...
    else if(isIrrelevantCommand(line))
    {
