    ...
    : m_file{std::make_unique<boost::iostreams::mapped_file_sink>(fn)}
{
  ...
  // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
  const auto data32 = reinterpret_cast<uint32_t*>(m_data);
...
void BlockData::processRgba(
  const uint32_t* src, const uint32_t blocks, const size_t offset, const size_t width, const bool useHeuristics)
{
  // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
  const auto dst = reinterpret_cast<uint64_t*>(m_data + m_dataOffset) + offset * 2u;
...
namespace
{
constexpr uint8_t expand6(const uint8_t value)
{
  // NOLINTNEXTLINE(hicpp-signed-bitwise)
  return (value << 2u) | (value >> 4u);
}

constexpr uint8_t expand7(const uint8_t value)
{
  // NOLINTNEXTLINE(hicpp-signed-bitwise)
...
// NOLINTNEXTLINE(bugprone-easily-swappable-parameters)
void decodeTAlpha(const uint64_t block, const uint64_t alpha, uint32_t* dst, const uint32_t w)
{
  ...
  const auto cr0 = gsl_lite::narrow_cast<uint8_t>(((rh0 << 6u) | (rl0 << 4u) | (rh0 << 2u) | rl0));
  const auto cg0 = gsl_lite::narrow_cast<uint8_t>((g0 << 4u) | g0);
  const auto cb0 = gsl_lite::narrow_cast<uint8_t>((b0 << 4u) | b0);

  const auto cr1 = gsl_lite::narrow_cast<uint16_t>((r1 << 4u) | r1);
  const auto cg1 = gsl_lite::narrow_cast<uint16_t>((g1 << 4u) | g1);
  const auto cb1 = gsl_lite::narrow_cast<uint16_t>((b1 << 4u) | b1);

  const auto codeword_hi = (block >> 2u) & 0x3u;
  const auto codeword_lo = block & 0x1u;
  const auto codeword = (codeword_hi << 1u) | codeword_lo;

  const auto base = gsl_lite::narrow_cast<int32_t>(alpha >> 56u);
  const auto mul = gsl_lite::narrow_cast<int32_t>((alpha >> 52u) & 0xFu);
  ...
  const std::array<uint32_t, 4> col_tab{{
    // NOLINTNEXTLINE(hicpp-signed-bitwise)
    gsl_lite::narrow_cast<uint32_t>(cr0 | (cg0 << 8u) | (cb0 << 16u)),
    // NOLINTNEXTLINE(hicpp-signed-bitwise)
    gsl_lite::narrow_cast<uint32_t>(c2r | (c2g << 8u) | (c2b << 16u)),
    // NOLINTNEXTLINE(hicpp-signed-bitwise)
    gsl_lite::narrow_cast<uint32_t>(cr1 | (cg1 << 8u) | (cb1 << 16u)),
    // NOLINTNEXTLINE(hicpp-signed-bitwise)
    gsl_lite::narrow_cast<uint32_t>(c3r | (c3g << 8u) | (c3b << 16u)),
  }};
...
// NOLINTNEXTLINE(bugprone-easily-swappable-parameters)
void decodeHAlpha(const uint64_t block, const uint64_t alpha, uint32_t* dst, const uint32_t w)
{
  const uint32_t indexes = (block >> 32u) & 0xFFFFFFFFu;
  ...
  const auto r0 = gsl_lite::narrow_cast<uint8_t>((r0444 << 4u) | r0444);
  const auto g0 = gsl_lite::narrow_cast<uint8_t>((g0444 << 4u) | g0444);
  const auto b0 = gsl_lite::narrow_cast<uint8_t>((b0444 << 4u) | b0444);

  const auto r1 = gsl_lite::narrow_cast<uint8_t>((r1444 << 4u) | r1444);
  const auto g1 = gsl_lite::narrow_cast<uint8_t>((g1444 << 4u) | g1444);
  const auto b1 = gsl_lite::narrow_cast<uint8_t>((b1444 << 4u) | b1444);
  ...
  const int32_t base = gsl_lite::narrow_cast<uint8_t>(alpha >> 56u);
  const int32_t mul = gsl_lite::narrow_cast<uint8_t>((alpha >> 52u) & 0xFu);
  const auto& tbl = getAlpha()[(alpha >> 48u) & 0xFu];

  const std::array<uint32_t, 4> col_tab{{
    // NOLINTNEXTLINE(hicpp-signed-bitwise)
    gsl_lite::narrow_cast<uint32_t>(clampu8(r0 + table59T58H[codeword]) | (clampu8(g0 + table59T58H[codeword]) << 8u)
                                    | (clampu8(b0 + table59T58H[codeword]) << 16u)),
    // NOLINTNEXTLINE(hicpp-signed-bitwise)
    gsl_lite::narrow_cast<uint32_t>(clampu8(r0 - table59T58H[codeword]) | (clampu8(g0 - table59T58H[codeword]) << 8u)
                                    | (clampu8(b0 - table59T58H[codeword]) << 16u)),
    // NOLINTNEXTLINE(hicpp-signed-bitwise)
    gsl_lite::narrow_cast<uint32_t>(clampu8(r1 + table59T58H[codeword]) | (clampu8(g1 + table59T58H[codeword]) << 8u)
...
// NOLINTNEXTLINE(bugprone-easily-swappable-parameters)
void decodePlanarAlpha(const uint64_t block, const uint64_t alpha, uint32_t* dst, const uint32_t w)
{
  ...
  const auto rh0 = gsl_lite::narrow_cast<uint8_t>((block >> (32u - 32u)) & 0x01u);
  // cppcheck-suppress duplicateExpression
  const auto rh1 = gsl_lite::narrow_cast<uint8_t>(((block >> (34u - 32u)) & 0x1Fu) << 1u);
  ...
  const auto chco = IVec16{
    gsl_lite::narrow_cast<int16_t>(rh - ro),
    gsl_lite::narrow_cast<int16_t>(gh - go),
    gsl_lite::narrow_cast<int16_t>(bh - bo),
    0,
    0,
    0,
    0,
  ...
  const auto cvco = IVec16{
    gsl_lite::narrow_cast<int16_t>((rv - ro) - 4u * (rh - ro)),
    gsl_lite::narrow_cast<int16_t>((gv - go) - 4u * (gh - go)),
    gsl_lite::narrow_cast<int16_t>((bv - bo) - 4u * (bh - bo)),
    0,
    0,
    0,
    0,
  ...
  auto col = IVec16{
    gsl_lite::narrow_cast<int16_t>(4u * ro + 2u),
    gsl_lite::narrow_cast<int16_t>(4u * go + 2u),
    gsl_lite::narrow_cast<int16_t>(4u * bo + 2u),
    0,
    0,
    0,
    0,
...
void decodeRgbaPart(uint64_t d, uint64_t alpha, uint32_t* dst, const uint32_t w)
{
  ...
  if(d & 0x2u)
  {
    const auto r0 = gsl_lite::narrow_cast<uint8_t>((d & 0xF8000000u) >> 27u);
    const auto g0 = gsl_lite::narrow_cast<uint8_t>((d & 0x00F80000u) >> 19u);
    const auto b0 = gsl_lite::narrow_cast<uint8_t>((d & 0x0000F800u) >> 11u);

    const auto dr = (static_cast<int32_t>(d) << 5u) >> 29u;
    const auto dg = (static_cast<int32_t>(d) << 13u) >> 29u;
    const auto db = (static_cast<int32_t>(d) << 21u) >> 29u;

    const int32_t r1 = static_cast<int32_t>(r0) + dr;
    const int32_t g1 = static_cast<int32_t>(g0) + dg;
    const int32_t b1 = static_cast<int32_t>(b0) + db;
  ...
  const std::array<uint32_t, 2> tcw{{
    gsl_lite::narrow_cast<uint32_t>((d & 0xE0u) >> 5u),
    gsl_lite::narrow_cast<uint32_t>((d & 0x1Cu) >> 2u),
  }};
