...
void ScreenOverlay::init(material::MaterialManager& materialManager, const glm::ivec2& viewport)
{
  *m_image = gl::Image<gl::PremultipliedSRGBA8>(viewport);
  ...
  auto texture = gsl_lite::make_shared<gl::Texture2D<gl::PremultipliedSRGBA8>>(m_image->getSize(), "screenoverlay");
  texture->assign(m_image->getData());
  m_texture = std::make_shared<gl::TextureHandle<gl::Texture2D<gl::PremultipliedSRGBA8>>>(
    texture,
    gsl_lite::make_unique<gl::Sampler>("screenoverlay" + gl::SamplerSuffix) | set(gl::api::TextureMinFilter::Nearest)
  ...
  m_mesh->bind("u_input",
               ...
               {
                 uniform.set(gsl_lite::not_null{m_texture});
               });
