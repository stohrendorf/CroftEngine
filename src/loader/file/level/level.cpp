...
void Level::postProcessDataStructures()
{
    ...
    for(const std::unique_ptr<SkeletalModelType>& model : m_animatedModels | boost::adaptors::map_values)
    {
        ...
        if(model->nMeshes > 1)
        {
            model->boneTree = gsl::make_span(
                reinterpret_cast<const BoneTreeEntry*>(&model->bone_index.from(m_boneTrees)), model->nMeshes - 1);
        }
    ...
    for(Animation& anim : m_animations)
    {
        ...
        {
            BOOST_LOG_TRIVIAL(warning) << "Pose frame data index "
                                       << anim.poseDataOffset.index<decltype(m_poseFrames[0])>() << " out of range 0.."
                                       << m_poseFrames.size() - 1;
            anim.frames = nullptr;
        }
        else
    ...
    for(Transitions& transition : m_transitions)
    {
        ...
        if(transition.transitionCaseCount > 0)
            transition.transitionCases = gsl::make_span(&transition.firstTransitionCase.from(m_transitionCases),
