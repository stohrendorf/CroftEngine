    ...
    const std::shared_ptr<render::scene::Material>& portalMaterial)
{
    ...
    resModel->getRenderState().setCullFaceSide(render::gl::CullFaceMode::Back);

    node = std::make_shared<render::scene::Node>("Room:" + std::to_string(roomId));
    node->setDrawable(resModel);
    node->addMaterialParameterSetter(
        "u_lightAmbient",
        [](const render::scene::Node& /*node*/, render::gl::Program::ActiveUniform& uniform) { uniform.set(1.0f); });
    node->addMaterialParameterSetter(
        "u_numLights",
    ...
    for(const SpriteInstance& spriteInstance : sprites)
    {
        ...
        spriteNode->addMaterialParameterSetter(
            ...
            [texture = sprite.texture](const render::scene::Node& /*node*/,
                                       render::gl::Program::ActiveUniform& uniform) { uniform.set(*texture); });
        spriteNode->addMaterialParameterSetter(
            "u_lightAmbient",
            [brightness = v.getBrightness()](const render::scene::Node& /*node*/,
                                             render::gl::Program::ActiveUniform& uniform) { uniform.set(brightness); });
