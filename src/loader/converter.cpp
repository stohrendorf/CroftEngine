...
void allocateElementMemory(const gsl::not_null<std::shared_ptr<gameplay::Mesh>>& mesh,
                           const gsl::not_null<aiMesh*>& outMesh)
{
...
void copyVertexData(const gsl::not_null<std::shared_ptr<gameplay::Mesh>>& mesh,
                    const gsl::not_null<aiMesh*>& outMesh)
{
...
template<typename T>
void copyIndices(const gsl::not_null<std::shared_ptr<gameplay::gl::IndexBuffer>>& buffer,
                 const gsl::not_null<aiMesh*>& outMesh,
                 size_t& indexOffset)
{
...
void copyIndices(const gsl::not_null<std::shared_ptr<gameplay::MeshPart>>& part,
                 const gsl::not_null<aiMesh*>& outMesh)
{
...
namespace loader
{
void Converter::write(const gsl::not_null<std::shared_ptr<gameplay::gl::Image<gameplay::gl::RGBA8>>>& srcImg,
                      size_t id) const
{
...
gsl::not_null<std::shared_ptr<gameplay::gl::Texture>> Converter::readTexture(const boost::filesystem::path& path) const
{
    {
    ...
    auto image = std::make_shared<gameplay::gl::Image<gameplay::gl::RGBA8>>( w, h,
                                                                             reinterpret_cast<const gameplay::gl::RGBA8*>(srcImage
                                                                                     .data()) );
    ...
    m_textureCache.emplace(std::make_pair(path, texture));
    return texture;
}

gsl::not_null<std::shared_ptr<gameplay::Material>> Converter::readMaterial(
        ...
        const gsl::not_null<std::shared_ptr<gameplay::ShaderProgram>>& shaderProgram) const
{
    ...
    auto material = make_not_null_shared<gameplay::Material>( shaderProgram );
...
gsl::not_null<std::shared_ptr<gameplay::Model>>
Converter::readModel(const boost::filesystem::path& path,
                     const gsl::not_null<std::shared_ptr<gameplay::ShaderProgram>>& shaderProgram,
                     const glm::vec3& ambientColor) const
{
    Assimp::Importer importer;
    const aiScene* scene = importer.ReadFile( (m_basePath / path).string(),
                                              aiProcess_JoinIdenticalVertices | aiProcess_Triangulate
                                              | aiProcess_ValidateDataStructure | aiProcess_FlipUVs );
    BOOST_ASSERT( scene != nullptr );

    auto renderModel = make_not_null_shared<gameplay::Model>();

    for( unsigned int mi = 0; mi < scene->mNumMeshes; ++mi )
    {
        BOOST_LOG_TRIVIAL( info ) << "Converting mesh " << mi + 1 << " of " << scene->mNumMeshes << " from "
                                  << m_basePath / path;
...

        {
            ...
            {
                vbuf[i].position = glm::vec3{mesh->mVertices[i].x, mesh->mVertices[i].y, mesh->mVertices[i].z}
                                   * static_cast<float>(SectorSize);
                if( mesh->HasNormals() )
                ...
                if( mesh->HasVertexColors( 0 ) )
                    vbuf[i].color = glm::vec4( mesh->mColors[0][i].r, mesh->mColors[0][i].g, mesh->mColors[0][i].b,
                                               mesh->mColors[0][i].a );
                else
        ...
        auto part = make_not_null_shared<gameplay::MeshPart>( builder.build( shaderProgram->getHandle() ) );
        ...
        renderModel->addMesh( to_not_null( renderMesh ) );
    }

    return renderModel;
}

void Converter::write(const gsl::not_null<std::shared_ptr<gameplay::Model>>& model,
                      ...
                      const glm::vec3& ambientColor) const
{
    ...
    if( formatIdentifier.empty() )
    {
        ...
        for( size_t i = 0; i < exporter.GetExportFormatCount(); ++i )
        {
            ...
            BOOST_LOG_TRIVIAL( info ) << descr->description << ", extension `" << descr->fileExtension << "`, id `"
                                      << descr->id << "`";
        }
    ...
    for( size_t mi = 0, globalPartIndex = 0; mi < model->getMeshes().size(); ++mi )
    {
        ...
        for( size_t pi = 0; pi < mesh->getPartCount(); ++pi, ++globalPartIndex )
        {
            ...
            const auto& part = mesh->getPart( pi );

            scene->mMeshes[globalPartIndex] = new aiMesh();
            auto outMesh = to_not_null( scene->mMeshes[globalPartIndex] );
            ...
            scene->mMaterials[globalPartIndex]
                    ->AddProperty( new aiColor4D( ambientColor.r, ambientColor.g, ambientColor.b, 1 ), 1,
...

            {
                ...
                auto finder = [&part](const Entry& entry) {
                    return entry.second == to_not_null( part->getMaterial() );
                };
                ...
                {
                    scene->mMaterials[globalPartIndex]->AddProperty(
                            new aiString( makeTextureName( texIt->first.tileAndFlag & TextureIndexMask ) + ".png" ),
    ...
    exporter.Export( scene.get(), formatIdentifier, fullPath.string(),
                     aiProcess_JoinIdenticalVertices | aiProcess_ValidateDataStructure | aiProcess_FlipUVs );
}
                      ...
                      const std::map<TextureLayoutProxy::TextureKey, gsl::not_null<std::shared_ptr<gameplay::Material>>>& mtlMap2) const
{
    ...
    for( const auto& room : rooms )
    {
        ...
        append( scene->mRootNode->mChildren, scene->mRootNode->mNumChildren, convert( *scene, room.sectors ) )->mName =
                room.node->getId() + ":boxes";
                           ...
                           const glm::vec3& ambientColor) const
{
    ...
    {
        convert( scene, *outNode, to_not_null( sourceModel ), mtlMap1, mtlMap2, ambientColor );
        hasContent = true;
    }
...
void Converter::convert(aiScene& scene,
                        aiNode& outNode,
                        const gsl::not_null<std::shared_ptr<gameplay::Model>>& inModel,
                        const std::map<TextureLayoutProxy::TextureKey, gsl::not_null<std::shared_ptr<gameplay::Material>>>& mtlMap1,
                        const std::map<TextureLayoutProxy::TextureKey, gsl::not_null<std::shared_ptr<gameplay::Material>>>& mtlMap2,
                        const glm::vec3& ambientColor) const
{
    for( const auto& inMesh : inModel->getMeshes() )
    {
        ...
        {
            const auto& inPart = inMesh->getPart( pi );

            append( outNode.mMeshes, outNode.mNumMeshes, scene.mNumMeshes );
            auto outMesh = to_not_null( append( scene.mMeshes, scene.mNumMeshes, new aiMesh() ) );
            ...
            outMaterial->AddProperty( new aiColor4D( ambientColor.r, ambientColor.g, ambientColor.b, 1 ), 1,
                                      AI_MATKEY_COLOR_AMBIENT );

            {
                ...
                auto finder = [&inPart](const Entry& entry) {
                    return entry.second == to_not_null( inPart->getMaterial() );
                };
