...
void DoneMenuState::handleObjectTick(engine::world::World& world, MenuDisplay& display, MenuObject& object)
{
...
std::unique_ptr<MenuState> DoneMenuState::tick(engine::world::World& world, MenuDisplay& display)
{
  ...
  case engine::TR1ItemId::Pistols:
    world.getPlayer().getInventory().tryUse(world.getObjectManager().getLara(),
                                            engine::TR1ItemId::Pistols,
                                            world.getEngine().getGameplayRules(),
  ...
  case engine::TR1ItemId::SmallMedipack:
    if(world.getPlayer().getInventory().tryUse(world.getObjectManager().getLara(),
...
void DoneMenuState::constructUi(ui::Ui& /*ui*/, engine::world::World& /*world*/, MenuDisplay& /*display*/)
{
}
} // namespace menu
